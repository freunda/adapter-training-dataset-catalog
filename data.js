...
window.CATALOG = [
  ...
  },
  {
    ...
    bestBaseline: "allenai/wildguard: 87.4% F1 response-harm; competitive with GPT-4 on refusal correctness",
    loraArtifacts: "allenai/wildguard (Mistral-7B FFT, LoRA-portable)",
    tier: 1,
    rank: 2,
    rankReason: "Fills Stage 2 Output Verification in Safety pipeline; 3-head design covers prompt-harm + response-harm + refusal-correctness in one training run; ODC-BY",
    taskCategories: ["Understanding"],
    taskNote: "Three classifiers in one: (1) is the prompt harmful? (2) is the response harmful? (3) is a refusal appropriate? Trains output-side safety gating — the complement to WildJailbreak's input-side guard.",
    notes: "Pairs with WildJailbreak: WildJailbreak covers input (jailbreak/injection), WildGuardMix covers output (harmful response, refusal correctness). The 3-head design lets one fine-tune serve three safety adapter tasks. Use `wildguardtest` split (gated, contact-email required) for held-out eval.",
    composability: "Stage 2 — output-side guardrail; runs after generation to classify response harm and verify refusal decisions",
    pipelineStage: 2,
    pipelineStageLabel: "Output Verification",
    avgContextTokens: 300,
    contextRelevancy: "High",
    contextRelevancyNote: "Both the prompt and response are required inputs — response harm is conditional on the prompt context.",
    trainableTasks: ["Response Harm Classification", "Refusal Correctness", "Prompt Harm Classification", "Output Safety Classification"]
  },
  {
    id: "aegis-2.0",
    name: "Aegis 2.0 (Nemotron Content Safety)",
    hf: "nvidia/Aegis-AI-Content-Safety-Dataset-2.0",
    domain: "LLM Safety",
    subdomain: "Harm Categorization",
    usage: "both",
    license: "CC BY 4.0",
    size: "33,416 annotated prompt+response pairs: 30,007 train / 1,445 val / 1,964 test / 12 hazard categories",
    bestBaseline: "nvidia/Llama-3.1-Nemoguard-8B-ContentSafety — competitive with WildGuard on response-harm classification",
    loraArtifacts: "nvidia/Llama-3.1-Nemoguard-8B-ContentSafety (full fine-tune, LoRA-portable)",
    tier: 1,
    rank: 3,
    rankReason: "Only CC BY 4.0 dataset with fine-grained 12-category harm taxonomy; pairs with WildGuardMix binary heads for a two-level safety stack; published Llama-3.1 fine-tune",
    taskCategories: ["Understanding", "Knowledge"],
    taskNote: "Multi-class harm categorization across 12 hazard categories (violence, self-harm, CBRN, etc.). Adds the 'which category' dimension on top of WildGuardMix's binary yes/no — enables policy-level routing.",
    notes: "Officially renamed 'Nemotron Content Safety Dataset V2'; 'Aegis 2.0' is the original name still used on HF. 12 top-level categories + 9 fine-grained subcategories. Labels from 3-LLM jury (Mixtral-8x22B, Mistral-NeMo-12B, Gemma-2-27B). Pairs with WildGuardMix: WildGuard for harm/refusal yes-no, Aegis for which category. Note: ~358 Suicide/Self-Harm samples come from a Kaggle dataset NOT redistributed here — see card for alignment instructions.",
    composability: "Stage 3 — harm categorization after Stage 2 binary detection; enables category-specific policy routing (e.g. CBRN → block, mild profanity → warn)",
    pipelineStage: 3,
    pipelineStageLabel: "Harm Categorization",
    avgContextTokens: 300,
    contextRelevancy: "High",
    contextRelevancyNote: "Both prompt and response are required to determine which harm category applies — harm type is context-dependent.",
    trainableTasks: ["Multi-class Harm Categorization", "Safety Policy Routing", "Fine-grained Content Classification"]
  }
];
